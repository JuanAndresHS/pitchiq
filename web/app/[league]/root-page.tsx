...
function LeagueCard({ card }: { card: Card }) {
  ...
  const metrics =
    league.slug === "cl" ? EUROPEAN_METRICS : LEAGUE_METRICS[league.slug];
...
export default async function HomePage() {
  // Champions League leads: it is the competition the other five feed into,
  // and the only page where their rating scales are compared.
  const cards = await Promise.all(
    [CHAMPIONS_LEAGUE, ...LEAGUES].map(buildCard),
  );
      ...
      <div className="mx-auto max-w-5xl px-6">
        <header className="border-pitch-line border-b pt-16 pb-10">
          ...
          <p className="text-pitch-dim mt-6 max-w-2xl text-lg leading-relaxed">
            Probabilistic match forecasts for Europe&apos;s five biggest leagues
            and the Champions League. Every fixture is a distribution, not a
            prediction — and every forecast is logged before kick-off so it can
          ...
          <section className="border-pitch-line mt-14 border-t pt-8">
            ...
            <div className="text-pitch-dim grid gap-6 text-sm leading-relaxed sm:grid-cols-3">
              ...
              <p>
                ...
                competition. A second model calibrates those scales against each
                other using European fixtures — the only matches that connect
                them.
              </p>
              <p>
